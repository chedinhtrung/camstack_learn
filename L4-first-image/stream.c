...
int main() {
    ...
    fmt.fmt.pix.pixelformat  = V4L2_PIX_FMT_YUYV;   // try MJPEG if YUYV unsupported
    ...
    // 6) Dequeue one filled buffer (blocks until a frame arrives)
    struct v4l2_buffer b = {0};
    b.type   = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    b.memory = V4L2_MEMORY_MMAP;
    if (xioctl(fd, VIDIOC_DQBUF, &b) < 0) { perror("VIDIOC_DQBUF"); return 1; }

    printf("Dequeued buffer #%i", b.index);
    ...
    fclose(f);
    printf("Saved %u bytes to frame.raw (w=%u h=%u fourcc=%c%c%c%c)\n",
           b.bytesused, fmt.fmt.pix.width, fmt.fmt.pix.height,
           fmt.fmt.pix.pixelformat & 0xFF,
           (fmt.fmt.pix.pixelformat >> 8) & 0xFF,
           (fmt.fmt.pix.pixelformat >> 16) & 0xFF,
           (fmt.fmt.pix.pixelformat >> 24) & 0xFF);

    // 8) (Optional) Requeue it if you wanted to keep streaming
